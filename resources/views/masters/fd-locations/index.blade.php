    ...
    <main class="main">
        <div class="page-shell">
            <div class="command-bar">
                <div class="crumb">Masters / FD Locations</div>
            </div>
            <div style="padding:12px;">
                <h1 class="title">FD Location Master</h1>
                ...
                <div class="card" style="margin-bottom: 12px;">
                    <div class="card-head">Add FD location</div>
                    <div style="padding: 12px;">
                        <form method="post" action="{{ route('masters.fd-locations.store', $companyQuery) }}">
                            @csrf
                            <div class="form-row">
                                <div>
                                ...
                                <div>
                                    <label for="fd_location_id">FD location ID</label>
                                    <input id="fd_location_id" name="fd_location_id" type="text" value="{{ old('fd_location_id') }}" required maxlength="100" placeholder="D365 FD location id">
                                </div>
                                <div>
                ...
                <div class="card">
                    <div class="card-head">FD locations</div>
                    <table>
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Company ID</th>
                                <th>FD location ID</th>
                                <th>Description</th>
                                <th>Created</th>
                                <th>Action</th>
                            </tr>
                        ...
                        <tbody>
                            @forelse ($fdLocations as $index => $row)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $row->company_id }}</td>
                                    <td>{{ $row->fd_location_id }}</td>
                                    <td>{{ $row->description ?? '—' }}</td>
                                    <td>{{ optional($row->created_at)->format('d M Y H:i') }}</td>
                                    <td>
                                        <form method="post" action="{{ route('masters.fd-locations.destroy', array_merge($companyQuery, ['fdLocation' => $row->id])) }}" onsubmit="return confirm('Delete this row?');" style="margin:0;">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="6">No FD locations found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
