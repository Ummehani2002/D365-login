...
class MembershipService
{
    ...
    public function createMembership(array $payload): CompanyMembership
    {
        ...
        $this->roleScopeService->validateRoleScopes($roleIds, is_array($roleScopes) ? $roleScopes : []);
        ...
        return DB::transaction(function () use ($payload, $roleIds, $roleScopes): CompanyMembership {
            ...
            if ($user) {
                $user->fill([
                    ...
                    'provider' => array_key_exists('provider', $payload) ? $payload['provider'] : ($user->provider ?? null),
                ]);
                $user->save();
            } else {
    ...
    public function updateMembership(CompanyMembership $membership, array $payload): CompanyMembership
    {
        ...
        return DB::transaction(function () use ($membership, $payload, $roleIdsProvided, $roleIds): CompanyMembership {
            ...
            if ($emailTaken) {
                throw ValidationException::withMessages(['email' => ['Another user already uses this email.']]);
            }

            $user->fill([
                'email' => $payload['email'],
                'name' => $payload['name'],
                'user_code' => array_key_exists('user_code', $payload) ? $payload['user_code'] : ($user->user_code ?? null),
                'provider' => array_key_exists('provider', $payload) ? $payload['provider'] : ($user->provider ?? null),
            ]);
            $user->save();
    ...
    private function validateRoleIds(array $roleIds): void
    {
        ...
        $count = Role::query()->whereIn('id', $roleIds)->count();
