...
class RbacMenuMatchController extends Controller
{
    ...
    {
        return response()->json([
            'menus' => $this->menuAccessService->listAvailableMenus(),
        ]);
    }
    ...
    public function assignMapping(AssignMenuPermissionMatchRequest $request): JsonResponse
    {
        try {
            ...
            return response()->json([
                'message' => $this->firstValidationError($exception) ?? 'Could not save mapping.',
        ...
        return response()->json([
            'mapping' => [
                ...
                'permission' => $match->permission ? [
                    'id' => $match->permission->id,
                    'slug' => $match->permission->slug,
                    'name' => $match->permission->name,
                ] : null,
            ],
        ]);
    }
