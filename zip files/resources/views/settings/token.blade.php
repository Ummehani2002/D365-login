...
<aside class="sidebar">
    ...
    <nav>
        ...
        <div class="nav-sub">
            ...
            <a class="nav-link" href="{{ route('settings.users.index') }}">Users</a>
            <a class="nav-link" href="{{ route('settings.roles.index') }}">Roles</a>
            <a class="nav-link" href="{{ route('settings.permissions.index') }}">Permissions</a>
            <a class="nav-link" href="{{ route('settings.menu-match.index') }}">Menu match</a>
        </div>
    </nav>
...
<main class="main">
    ...
    <div class="card">
        <div class="card-title">
            Bearer Token
            @if(! $hasD365Credentials)
                <span class="badge badge-none">Credentials required</span>
        ...
        @php
            $totalSeconds = 3599;
            $remaining    = $hasD365Credentials && $token ? $token->secondsRemaining() : 0;
...
<script>
    ...
    const HAS_D365_CREDS = @json($hasD365Credentials);
    ...
    function triggerAutoGenerate() {
        if (!HAS_D365_CREDS) {
            updateRing(0);
            setAutoStatus('Save D365 credentials before generating a token.', '#605e5c', '#f3f2f1');
            return;
        }
    ...
    if (!HAS_D365_CREDS) {
        updateRing(0);
        setAutoStatus('D365 credentials are not configured. Timer is paused and no token API calls will run.', '#605e5c', '#f3f2f1');
    } else if (SECS_AT_LOAD > 0) {
