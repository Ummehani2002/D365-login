...
class RoleScopeService
{
    ...
    {
        $membership->loadMissing(['roles:id,name', 'roleScopes.companies:id,d365_id,name']);
    ...
    public function validateRoleScopes(array $roleIds, array $roleScopes): void
    {
        ...
        $selectedRoleIds = collect($roleIds)
        ...
        foreach ($roleScopes as $scope) {
            ...
            $allOrganizations = (bool) ($scope['all_organizations'] ?? false);
            $companyIds = collect($scope['company_ids'] ?? [])->filter()->all();
            if (! $allOrganizations && count($companyIds) === 0) {
                throw ValidationException::withMessages([
                    'role_scopes' => ['Choose at least one company when using selected organizations.'],
                ]);
            }
        }
    }
    ...
    public function syncScopesForMembership(CompanyMembership $membership, array $roleIds, array $roleScopes): void
    {
        ...
        foreach ($roleIds as $roleId) {
            ...
            if (! $allOrganizations && $companyIds === []) {
                throw ValidationException::withMessages([
                    'role_scopes' => ['Choose at least one company for each selected role.'],
                ]);
            }
            ...
            if ($allOrganizations) {
                $scope->companies()->sync([]);
            } else {
                $scope->companies()->sync($companyIds);
            }
        }
    }
    ...
    public function pruneScopesByRoleIds(CompanyMembership $membership, array $roleIds): void
    {
        ...
        foreach ($roleIds as $roleId) {
            $scope = CompanyMembershipRoleScope::query()->firstOrCreate(
                [
                    'company_membership_id' => $membership->id,
                    'role_id' => (int) $roleId,
                ],
            ...
            if (! $scope->all_organizations && $scope->companies()->doesntExist() && $defaultCompanyIds !== []) {
                $scope->companies()->sync($defaultCompanyIds);
            }
        }
    }
    ...
    public function upsertRoleScope(CompanyMembership $membership, array $payload): CompanyMembershipRoleScope
    {
        ...
        if (! $hasRole) {
            throw ValidationException::withMessages([
                'role_id' => ['Assign the role to the user first.'],
            ]);
        }

        if (! $allOrganizations && count($companyIds) === 0) {
            throw ValidationException::withMessages([
                'company_ids' => ['Choose at least one company when using selected organizations.'],
            ]);
        }
